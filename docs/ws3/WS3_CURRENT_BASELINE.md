...
**최종 업데이트**: 2026-05-20  
**기능 단계 (current functional baseline)**: WS3 v0.32.3 백서 카드 슬롯 + v2 제품형 카드 UI + 빗썸 메인 정렬 code-ready (백서 §16 카드 슬롯 + v2 카드 표현 + 빗썸 메인 + reasonChips 한글 매핑 / 기능 변경 0건 / Worker 미수정 — v2 카드 shell 이식 `.ws3-result-card` (.grade-SP gold/.grade-S pink/.grade-A red/.grade-B orange/.grade-C muted 좌측 네온 라인 4px + soft bg gradient) + `.ws3-card-header` (시안 dot + 코인명 + 큰 점수 cyan) + `.ws3-card-chips` (등급/검토레벨/후보·검토/거래소/한글 reasonChips chip variant 17종 — v2 hdr-chip 색 .exchange-bit/upb/bin 그대로) + `.ws3-signal-bar` (cyan→green gradient + glow) + 카드 메타 row + `.ws3-card-body-toggle` 펼침 + 백서 §16.4 18 섹션 카드바디 slot (구현 8개: 핵심 요약/카드헤더 해석/점수 요약/선별 근거·종합 브리핑/강점·약점/구조 판단/매수세·수급/지표 상세/고급 상세 · 후속 10개: 시장상황·전략성향·진입가이드·매도전략 A-F 6 slot·세력판단·반복신호·코인메타·뉴스·시장참고·외부보조·후보검증 — 모두 "데이터 수집 중"/"계산 대기"/"후속 단계" 안내, 섹션 삭제 0건) + 섹션 mini toggle. 신규 JS 함수 9개: `WS3_CHIP_KR_MAP`/`ws3MapChipKr`/`ws3RenderChipsKr`/`ws3ExchangeChipClass`/`ws3ExchangeLabel`/`ws3OrLevelKr`/`ws3GradeChipClass`/`ws3GradeLabel`/`ws3SplitStrengthsWeaknesses`/`ws3BuildBriefing`/`ws3BuildCardBody`. reasonChips 한글 매핑 7종 (VOLUME_SURGE→거래량 증가 / LOW_VOLUME→거래량 부족 / HIGH_CLOSE_POSITION→종가 상단 / UPPER_WICK_RISK→윗꼬리 리스크 / WIDE_RANGE_RISK→변동폭 위험 / SHORT_MOMENTUM→단기 모멘텀 / POSITIVE_CHANGE→상승 전환). 빗썸 메인 정렬: 거래소 select 3 spots 순서 bithumb(메인)/upbit(보조 selected)/binance(참고) + label "(빗썸 메인 · 업비트 보조 · 바이낸스 참고)" + Dev Open Banner 상태 배지 4종 (준비/개발 개방 모드/수동 운영/자동 알림 꺼짐) + "메인 거래소: 빗썸" 강조 + chip 색 v2 .exchange-bit #ff7849 / .exchange-upb #5da9ff / .exchange-bin #f0b90b. mcRenderResults 전면 교체 (v2 카드 shell + 백서 18 섹션). mcRenderTop5 한글 chip 적용. Section 11 llRefreshSelectedPreview 한글 라벨 + ws3MapChipKr chip 적용 (선택 마켓/거래소·기준봉/점수·등급/운영자 검토 레벨/후보 자격/근거(한글)/최근 시각/종가/변화율·거래량 배수/위험 메모). v2 데이터 의미 (관심/정밀/표준/24h/진행도/active tracking/completed history/분석기 state pipeline) 미이식, WS3 독립 앱 유지. SPLUS/splus 사용자 노출 0건 (코드 주석에서도 제거). 도지/망치/장악형 사용자 UI + 코드 주석 매치 0건 — 구조값 표현만. 정밀/표준/24h/진행도 매치 0건. WS3_LIMITED_LIVE_ENABLED=true 운영 유지 / VERSION `WS3_v0.31.0_web_first_minimum_operator_mode` 유지 / Worker auth dev-open 그대로 / Cron 0건 / 자동 알람 0건 / candidate 저장 0건 / tracking 0건. v0.32.2 v2 UI 톤 이식 + 한글 우선 문구 정렬 (백서 정렬 / 기능 변경 0건 / Worker 미수정 — v2 디자인 톤(다크 네이비 `#080d13` / 글래스 카드 `#0f1a27` / 시안 네온 `#00d4ff` / 32px 그리드 배경 / glow / JetBrains Mono + Noto Sans KR) 이식, v2 데이터 의미·active tracking·completed history·분석기 state pipeline은 미이식 (WS3 독립 앱 유지). 사용자 UI 영어 라벨 한글 우선 정리 — 헤더 11개 / sub-panel 헤더 / input label / button label / 안내문 / dashboard card label 모두 한글. Worker raw key (mode/messageType/code/score/grade/reasonChips 등) 그대로 (디버깅/매핑 일관성). 영어 병기 정책 — 운영자 검토(Operator Review) / 제한 운영(Limited Live) / 개발 개방(Dev Open). SPLUS / splus / 도지 / 망치 / 장악형 사용자 UI 0건. 구조값 chip (VOLUME_SURGE / HIGH_CLOSE_POSITION / LOW_VOLUME / UPPER_WICK_RISK) 유지. P-S/P-A/P-B/P-C 등급 코드 그대로. WS3_LIMITED_LIVE_ENABLED=true 운영 유지 / Worker auth dev-open 그대로 / Cron 0건 / 자동 알람 0건 / candidate 저장 0건 / tracking 0건. v0.32.1 Dev Open Operator UX — Live Validation Success (속도 우선 / 보안은 final deploy 게이트 deferred — Worker `isDevOpenOperatorRoute` helper + `WS3_OPERATOR_AUTH_MODE='DEV_OPEN'` 상수 추가 / 5 dev-open routes (/state, /multi-candidate-dry-run, /send-limited-live-alert, /live-preflight, /candidate-dry-run)는 Invoke Token check skip — token 없이 호출 가능 / 4 잠금 routes (/send-canary, /cleanup-confirm, /operator-reset, /send-candidate-test)는 token check 그대로 (잘못 누른 운영자 안전 차단) / Web Console 사용자 token UI 입력 0건 — Section 1 Configuration `Worker Endpoint` / `Invoke Token` text input 제거 (hidden 으로만 back-compat) / Dev Open Banner 추가 ("Operator Mode: DEV OPEN | Invoke Token input disabled | Endpoint preset") / `WS3_DEFAULT_WORKER_ENDPOINT` preset 상수 / safeEndpoint→preset 반환 / readTokenAndClear→null / postJson·getJson token이 null이면 header omit / invite gate hidden 우회 (코드 보존, final security 게이트에서 재활성화 용이) / [A] textarea label "최대 10" → "최대 50" 통합 / Worker VERSION `WS3_v0.31.0_web_first_minimum_operator_mode` 유지 / KV write scope (CANDIDATE_TEST_GUARD_ONLY / LIMITED_LIVE_GUARD_ONLY 등) 그대로 / Telegram fixed-text path / duplicate guard / confirmPhrase / env-gate (WS3_LIMITED_LIVE_ENABLED) 그대로 / 5 env vars 그대로 (WS3_LIMITED_LIVE_ENABLED='true' 운영 유지) / Cron 0건 / 자동 알람 0건 / candidate 저장 0건 / tracking 시작 0건 / 노출된 폐기 hash repo-wide 매치 0건 / bot_token / chat_id / message_id / Invoke Token / SHA-256 hash / KV namespace ID / raw Telegram response / raw exchange full response 모두 정책 문맥만 / 본 commit 시점 Cloudflare 변경 0건 / 실 호출 0건 — final security (Cloudflare Access / Pages Function proxy / server-side token custody / invite gate 재활성화 / role separation) 는 별도 게이트로 deferred. v0.32.0 자연검증 PASS 후속: Limited Live Operation Enabled 유지 / Web Console UI 대규모 보강 — Operator Dashboard Summary 3-card HOT/WATCH/LOW + last scan meta + operator review total / Top 5 Fixed Candidates 별도 panel / Failed Markets List client-side 추출 / Filter Tabs ALL/HOT_REVIEW/WATCH_REVIEW/LOW_SIGNAL/VOLUME_SURGE/HIGH_CLOSE_POSITION/isCandidate 7종 + count badge / Section 11 Selected Candidate Preview + Telegram Message Preview (client-side display only, API 호출 0건) + Recent LIMITED LIVE Sent History (max 5 memory-only) / Memory-only Scan History row 확장 (okCount/failCount/HOT/WATCH/LOW counts) / 모바일 UX 개선 (dashboard gap / tab min-width / danger-zone row spacing) / Worker 미수정 / VERSION `WS3_v0.31.0_web_first_minimum_operator_mode` 유지 / Worker redeploy 불필요 / 보호 파일 diff 0건 / Cron 0건 / 자동 알람 0건 / candidate 저장 0건 / tracking 시작 0건 / 노출된 폐기 hash repo-wide 매치 0건 / raw Telegram response / raw exchange full response / Invoke Token / KV namespace ID / bot_token / chat_id / message_id 노출 0건)  
...
| **WS3 v0.32.0** | **`/web/ws3-canary-console.html` (보강 1739→2267 +528) + `/web/ws3-canary-console/index.html` (mirror) + `/docs/ws3/WS3_v0_32_0_V2_GRADE_OPERATOR_CONSOLE_REPORT.md`** | **`519b094`** | **✅ 박제 (V2-grade Operator Console Fast Track Pack — 자동화 아님 / 운영자 수동 판단력 v2-grade 도약 UX Pack. Worker 미수정 (`workers/ws3-telegram-canary-worker.js` diff 0건, VERSION `WS3_v0.31.0_web_first_minimum_operator_mode` 유지) / Web-only 보강 9 묶음: (1) Section 8 Operator Dashboard Summary — 3-card grid (HOT_REVIEW red / WATCH_REVIEW orange / LOW_SIGNAL gray) + last scan meta + operator review total (HOT+WATCH) + candidate count, (2) Section 8 Top 5 Fixed Candidates 별도 panel (rank/market/grade/score/lvl badge/[CAND]/[OR]/chips/latestTime/lastClose, Worker 정렬 그대로 사용), (3) Section 8 Failed Markets List (client-side `results.filter(r => r.ok === false)` 추출 — Worker 응답에 이미 ok=false rows 포함, Worker response 보강 0건), (4) Section 8 Filter Tabs 7종 (ALL/HOT_REVIEW/WATCH_REVIEW/LOW_SIGNAL/VOLUME_SURGE/HIGH_CLOSE_POSITION/isCandidate) + count badge / 새 scan마다 ALL로 reset / mcRenderResults filter 적용, (5) Section 11 Selected Candidate Preview (market/exchange/timeframe/score/grade/level/isCandidate/operatorReview/chips/latestTime/lastClose/changePct/volRatio/risk note), (6) Section 11 Telegram Message Preview (client-side display only — `[WOOS WS3 LIMITED LIVE / OPERATOR REVIEW]` 5 안전 라벨 + Market/Exchange/Timeframe/Score/Grade/level/isCandidate/chips/candidateStored:false/trackingStarted:false / eligibility 검사 client-side 안내 / 매수·진입·수익·LIVE 금지 문구 0건 / API 호출 0건), (7) Section 8 Memory-only Scan History row 확장 (okCount/failCount/HOT/WATCH/LOW counts 추가), (8) Section 11 Memory-only Sent History (`ws3SentHistory[]` max 5, code=LIMITED_LIVE_REVIEW_SENT + safety.telegramSent=true 시에만 append, localStorage/sessionStorage/indexedDB/cookie 사용 0건), (9) 모바일 UX (dashboard gap 축소 / tab min-width 56px / danger-zone row 14px margin / dashboard card 14px val) + 신규 50+ CSS 클래스 `.ws3-dash-grid` `.ws3-dash-card` `.ws3-tabs` `.ws3-tab` `.ws3-top5-row` `.ws3-failed-row` `.ws3-sent-row` `.ws3-preview-box` `.ws3-preview-empty` `.ws3-selected-card`. 신규 JS 함수 11+ (`ws3MatchesFilter` / `mcCountFilterMatches` / `mcInitFilterTabs` / `mcApplyFilter` / `mcRenderDashboard` / `mcRenderTop5` / `mcRenderFailed` / `llRefreshSelectedPreview` / `llRefreshTelegramPreview` / `llAppendSentHistory` / `llRenderSentHistory`), 신규 state vars (`ws3CurrentFilter='ALL'` / `lastMultiRunBody=null` / `ws3SentHistory=[]`). 필수 정적 테스트: node --check Worker PASS / web↔index mirror 0건 / embedded `<script>` 2 blocks parse OK (4428 + 74798 chars) / storage grep 매치 2건 모두 정책 문맥 (실 호출 0건) / hash·KV ID·token·message_id·raw response leak 0건 / 보호 파일 diff 0건. 본 commit 까지 Cloudflare Worker redeploy 0건 / Pages redeploy 0건 / Telegram API 0건 / KV write 0건 / candidate 저장 0건 / tracking 시작 0건 / 실 거래소 API 0건. Cron / 자동 Telegram / candidate 저장 / tracking 시작 — 모두 계속 disabled. v0.32+ 자연검증 후속: 모바일 dashboard 가독성 / Top 5 wrap / tab wrap / preview 폰트 / preset failCount 추가 감소 / operatorReview 임계값 조정 / duplicate guard window 적정성)** |
| **WS3 v0.32.1** | **`/workers/ws3-telegram-canary-worker.js` (보강) + `/web/ws3-canary-console.html` (보강) + `/web/ws3-canary-console/index.html` (mirror) + `/docs/ws3/WS3_v0_32_1_NO_INVOKE_TOKEN_DEV_OPEN_OPERATOR_UX_REPORT.md` + Live Validation Closure (`c6d06d6`)** | **`20e632d` / `c6d06d6`** | **✅ 박제 (No Invoke Token / Dev Open Operator UX Patch — 속도 우선 / 보안은 final deploy 게이트 deferred. Worker 신규 상수 `WS3_OPERATOR_AUTH_MODE='DEV_OPEN'` + helper `isDevOpenOperatorRoute(pathname)` (returns true for /state, /multi-candidate-dry-run, /send-limited-live-alert, /live-preflight, /candidate-dry-run). 5 dev-open routes의 기존 token check 3-block (missing / env-missing / mismatch) 을 `if (!isDevOpenOperatorRoute(path)) { ... }` 로 wrap — 기존 코드 보존하여 final security 게이트에서 즉시 재활성화 용이. 4 잠금 routes (/send-canary 1766 / /cleanup-confirm 1999 / /operator-reset 2087 / /send-candidate-test 2474) token check 그대로 (잘못 클릭한 운영자 안전 차단). Web Console 사용자 token UI 입력 0건: Section 1 Configuration `Worker Endpoint` (type=text) + `Invoke Token` (type=password) input 제거 → panel display + `<input type="hidden">` 으로만 back-compat / Dev Open Banner 추가 ("Operator Mode: DEV OPEN | Invoke Token input disabled | Endpoint preset") / `<main hidden>` → `<main>` (접속 즉시 main 표시) / `<section id="inviteGate" ... hidden>` (코드 보존, final 게이트 재활성화 용이) / `WS3_DEFAULT_WORKER_ENDPOINT` preset 상수 / `safeEndpoint()` → preset endpoint 반환 / `readTokenAndClear()` → null 반환 (no-op) / `postJson`·`getJson` token이 string && length > 0일 때만 X-WS3-Canary-Token header set / 5 dev-open button handler (checkBtn / lpRunBtn / cdrRunBtn / mcRunBtn / llSendBtn) 에서 `if (!token) MISSING_INVOKE_TOKEN` 제거 / 4 잠금 button handler (sendBtn / cleanupBtn / operatorResetBtn / ctSendBtn)은 그대로 — token=null → client-side 즉시 차단. [A] textarea label "Markets (콤마 또는 줄바꿈, 최대 10)" → "최대 50" 통합 (v0.32 자연검증 minor finding, v0.31.0-fix-1 cap=50 갱신 시 누락분). VERSION `WS3_v0.31.0_web_first_minimum_operator_mode` 유지 (response shape 변경 0건) / KV write scope (CANDIDATE_TEST_GUARD_ONLY / LIMITED_LIVE_GUARD_ONLY / alreadySent / cleanupRequired / operatorReset / invokeTokenMismatch) 그대로 / Telegram fixed-text path / duplicate guard / confirmPhrase / env-gate (WS3_LIMITED_LIVE_ENABLED) 그대로 / 5 env vars 그대로 (WS3_LIMITED_LIVE_ENABLED='true' 운영 유지) / Cron 0건 / 자동 알람 0건 / candidate 저장 0건 / tracking 시작 0건. 필수 정적 테스트: node --check Worker PASS / web↔index mirror 0건 / embedded `<script>` 2 blocks parse ALL_BLOCKS_OK / storage grep 매치 2건 모두 정책 문맥 (실 호출 0건) / token UI 사용자 입력 검사 0건 (hidden 으로만 back-compat) / hash·KV ID·token·message_id·raw response leak 0건 / 보호 파일 diff 0건. 본 commit 까지 Cloudflare 변경 0건 / 실 호출 0건. final security TODO (별도 게이트): Cloudflare Access / Pages Function proxy / server-side token custody / invite gate 재활성화 / role separation. 자연검증 누적 (코드 변경 X): [B] failCount=3 (KRW-MANA·KRW-AAVE·+1, 자연 변동 가능) / [C] WATCH_REVIEW=7 / LOW_SIGNAL=22 (24%, 운영자 부담 적정))** |
| **WS3 v0.32.2** | **`/web/ws3-canary-console.html` (보강) + `/web/ws3-canary-console/index.html` (mirror) + `/docs/ws3/WS3_v0_32_2_V2_UI_TONE_KOREAN_ALIGNMENT_REPORT.md`** | **`1c47161`** | **✅ 박제 (v2 UI 톤 이식 + 한글 우선 문구 정렬 — 백서 정렬 / 기능 변경 0건 / Worker 미수정. v2 디자인 톤 이식 (root `index.html` read-only 참고): 다크 네이비 `#080d13` + 글래스 카드 `#0f1a27` + 시안 네온 `#00d4ff` + 32px×32px 시안 그리드 배경 + glow-cyan / glow-green / glow-red / glow-orange + JetBrains Mono + Noto Sans KR + radius 6px + `:root` CSS 변수 21개 + button variant (primary cyan glow / danger red glow / controlled yellow) + status 색상 (READY cyan / OK green / ERROR red / BLOCKED orange) + 텍스트 hierarchy (text-primary/secondary/muted) + .ws3-dash-card (.hot red glow / .watch orange glow / .low muted) + .ws3-tab .active cyan glow + .ws3-preview-box cyan dashed border + .ws3-selected-card cyan-tinted glass + 모바일 @media 확장. v2 데이터 의미·active tracking·completed history·분석기 state pipeline·관심/정밀/표준/24h 퍼센트/진행도 의미 미이식 (WS3 독립 앱 유지). 사용자 UI 영어 라벨 한글 우선 정리 — h1 WS3 운영 콘솔 / Section 헤더 11개 (1. 설정 / 2. 상태 / 3. 정리 확인 / 4. 위험 영역 / 5. 결과 / 6. 시세 미리보기 / 7. 단일 코인 점수 / 8. 다종목 스캔 / 9. 테스트 알림 발송 / 10. 제한 운영 모드 / 11. 운영자 검토 발송) / Sub-panel 헤더 (운영 요약 / 상위 후보 5개 / 후보 목록 / 실패 마켓 / 필터 / 최근 스캔 기록 / 선택 후보 미리보기 / 텔레그램 미리보기 / 최근 발송 기록) / Dashboard card HOT_REVIEW/WATCH_REVIEW/LOW_SIGNAL → HOT/WATCH/LOW (badge 축약) / 모든 input label 한글 (거래소 / 마켓 / 마켓 목록 / 타임프레임 / 캔들 수 / 확인 문구 / 리셋 문구 / 강제 테스트 사유 / 검토 대상 카드) / 모든 button label 한글 (상태 확인 / 정리 확인 / 카나리 발송 / 운영자 리셋 / 시세 미리보기 실행 / 단일 코인 점수 실행 / 스캔 실행 / 업비트 원화 프리셋 불러오기 / 테스트 알림 발송 / 제한 운영 알림 보내기) / 안내문 + footnote + warn 한글 / JS statusEl 'status:'→'상태:' / Section 11 selector placeholder 한글 / Worker raw key (mode/messageType/code/score/grade/reasonChips/telegramSent/kvWritten/kvWriteScope/candidateStored/trackingStarted 등) 그대로 (디버깅·매핑 일관성) / 영어 병기: 운영자 검토(Operator Review) / 제한 운영(Limited Live) / 개발 개방(Dev Open). 백서 기준 준수: SPLUS / splus / 도지 / 망치 / 장악형 사용자 UI 0건 (grep 검증), 구조값 chip (VOLUME_SURGE / HIGH_CLOSE_POSITION / LOW_VOLUME / UPPER_WICK_RISK) 유지, P-S/P-A/P-B/P-C 등급 코드 그대로, WS3 독립 앱 유지. 기능 / Worker / KV / Telegram / env / route 모두 미변경. 보호 파일 (workers/ / worker.js / wrangler.toml / index.html / manifest.json / service-worker.js / v3/ / CODE_CONTRACT / WORKFLOW_TEMPLATE / wrangler-canary.toml / workers/ws3-canary-state-kv-adapter.js / wrangler-canary.example.toml / .gitignore) diff 0건 / VERSION `WS3_v0.31.0_web_first_minimum_operator_mode` 유지 / 5 env vars 그대로 (LIMITED_LIVE_ENABLED='true' 운영 유지) / Cron 0건 / 자동 알람 0건 / candidate 저장 0건 / tracking 0건. 필수 정적 테스트: diff -q web↔index 0건 / embedded `<script>` 2 blocks parse ALL_BLOCKS_OK / 영어 UI label 잔존 grep 0건 / SPLUS·도지·망치·장악형 grep 0건 / 노출된 폐기 hash repo-wide 매치 0건 / token·KV ID·message_id·raw response 정책 문맥만 / 매수·진입·수익·LIVE 금지 문구 0건 / Cloudflare 변경 0건 / 실 호출 0건. 후속: push 승인 후 Pages redeploy 1회만 (Worker 미수정), 자연검증 — console 진입 → 32-market scan → v2 톤 / 한글 문구 / dashboard / top5 / filter / preview 확인. 다음 후보: Bithumb 전환 / 등급 정렬 / structureBucket / signalCycle / 자연검증 후 톤·문구 미세 조정)** |
| **WS3 v0.32.3** | **`/web/ws3-canary-console.html` (보강) + `/web/ws3-canary-console/index.html` (mirror) + `/docs/ws3/WS3_v0_32_3_WHITEPAPER_CARD_SLOT_V2_UI_REPORT.md`** | **(push 후 기록)** | **✅ 박제 (백서 카드 슬롯 + v2 제품형 카드 UI + 빗썸 메인 정렬 — 기능 변경 0건 / Worker 미수정. 카드 내용 = 백서 §16 / 카드 표현 = v2 스크린샷 (root index.html 참고) / 운영 기준 = 빗썸 메인 + 한글 우선. 신규 v2 카드 CSS: `.ws3-result-card` (.grade-SP gold #ffd233 / .grade-S pink #ff7ad9 / .grade-A red / .grade-B orange / .grade-C muted) 좌측 네온 라인 4px + soft bg gradient / `.ws3-card-header` (시안 dot + glow / 코인명 mono / 큰 점수 cyan) / `.ws3-card-chips` + `.ws3-chip` variant 17종 (grade-SP/S/A/B/C, mode-hot/watch/low, candidate, exchange-bit/upb/bin, structure, volume, risk, neutral, time) — v2 hdr-chip 색 그대로 / `.ws3-signal-bar` + `.ws3-signal-fill` (cyan→green gradient + glow) / `.ws3-card-meta` (mono muted) / `.ws3-card-body-toggle` 펼침 + `.ws3-card-body` (.expanded) / `.ws3-card-section` (.collapsed) + `.toggle-mini` 섹션 mini toggle / `.strong` green / `.weak` red / `.dim` muted. 신규 JS 함수 9개: WS3_CHIP_KR_MAP / ws3MapChipKr / ws3RenderChipsKr / ws3ExchangeChipClass / ws3ExchangeLabel / ws3OrLevelKr / ws3GradeChipClass / ws3GradeLabel / ws3SplitStrengthsWeaknesses / ws3BuildBriefing / ws3BuildCardBody. reasonChips 한글 매핑 7종 (VOLUME_SURGE→거래량 증가 / LOW_VOLUME→거래량 부족 / HIGH_CLOSE_POSITION→종가 상단 / UPPER_WICK_RISK→윗꼬리 리스크 / WIDE_RANGE_RISK→변동폭 위험 / SHORT_MOMENTUM→단기 모멘텀 / POSITIVE_CHANGE→상승 전환). 빗썸 메인 정렬: 거래소 select 3 spots (Section 6/7/8) 순서 bithumb(메인)/upbit(보조 selected)/binance(참고) + label "(빗썸 메인 · 업비트 보조 · 바이낸스 참고)" + Dev Open Banner 상태 배지 4종 (준비 / 개발 개방 모드 / 수동 운영 / 자동 알림 꺼짐) + "메인 거래소: 빗썸" 강조 line + chip 색 v2 .exchange-bit #ff7849 / .exchange-upb #5da9ff / .exchange-bin #f0b90b 그대로. mcRenderResults 전면 교체: 카드 헤더 (좌측 네온 라인 + dot + 코인명 + 큰 점수 + chip 묶음 + 신호 강도 바 + 메타 row) + 카드바디 §16.4 18 섹션 모두 slot 표시 (구현 8: §1 핵심 요약 / §2 카드헤더 해석 / §3 점수 요약 + 신호 바 / §3.5 선별 근거·종합 브리핑 / 강점·약점 / §4 구조 판단 / §5 매수세·수급 / §12 지표 상세 / §18 고급 상세 · slot 표시 10: §6 시장상황·BTC / §7 전략성향 / §8 진입 가이드 / §9 매도전략 A-F 6 slot / §10 세력 판단 / §11 반복신호 / §13 코인 메타 / §14 뉴스 / §15 시장 참고 / §16 외부 보조 / §17 후보 검증 — 모두 "데이터 수집 중"/"계산 대기"/"후속 단계" 안내, 섹션 삭제 0건) + 펼침 토글 + 섹션 mini toggle. mcRenderTop5 한글 chip 적용. Section 11 llRefreshSelectedPreview 한글 라벨 + ws3MapChipKr 한글 chip 적용 (선택 마켓 / 거래소·기준봉 / 점수·등급 / 운영자 검토 레벨 / 후보 자격 예/아니요 / 운영자 검토 예/아니요 / 근거(한글) / 최근 시각 / 종가 / 변화율·거래량 배수 / 위험 메모 한글). v2 데이터 의미 (관심/정밀/표준/24h/진행도/active tracking/completed history/분석기 state pipeline) 미이식, WS3 독립 앱 유지. SPLUS/splus 사용자 노출 0건 (코드 주석에서도 제거). 도지/망치/장악형 사용자 UI + 코드 주석 매치 0건 (코드 주석 "차트 패턴명 사용 금지 — 구조값 표현만" 으로 갱신). 정밀/표준/24h/진행도 매치 0건. 기능 / Worker / KV / Telegram / env / route 모두 미변경. 보호 파일 (workers / worker.js / wrangler.toml / index.html / manifest.json / service-worker.js / v3/ / CODE_CONTRACT / WORKFLOW_TEMPLATE / KV adapter / wrangler-canary.example.toml / .gitignore) diff 0건. VERSION `WS3_v0.31.0_web_first_minimum_operator_mode` 유지 / 5 env vars 그대로 (WS3_LIMITED_LIVE_ENABLED='true' 운영 유지). 필수 정적 테스트: diff -q web↔index 0건 / embedded `<script>` 2 blocks parse ALL_BLOCKS_OK / 영어 UI label 잔존 grep 0건 / SPLUS·도지·망치·장악형 grep 0건 / 정밀·표준·24h·진행도 grep 0건 / 노출된 폐기 hash repo-wide 매치 0건 / bot_token·chat_id·message_id·Invoke Token·SHA-256 hash·KV namespace ID·raw Telegram response·raw exchange full response 정책 문맥만 / 매수·진입·수익·LIVE 금지 문구 0건 / Cloudflare 변경 0건 / 실 호출 0건. 후속: push 후 Pages redeploy 1회 (Worker 미수정), 자연검증 — console 진입 → 32-market scan → v2 카드 shell / 백서 §16 슬롯 / 빗썸 메인 / 한글 chip / 카드 펼침 / 종합 브리핑 / 강점·약점 시각 확인. 다음 후보: 상단 탭(스캐너/추적 분석) / 검색창 + 정렬 버튼 / 큰 "스캔 시작" 중심 버튼 / 빗썸 adapter 신규 구현 / 카드바디 미구현 모듈 점진 구현 / S+ 등급 정렬 / structureBucket / signalCycle / Cron·auto Telegram·candidate 저장·tracking 시작은 별도 사용자 명시 승인 전까지 계속 disabled)** |
...
```text
- v0.32.3 = 백서 §16 카드 슬롯 + v2 제품형 카드 UI + 빗썸 메인 정렬. 기능 변경 0건. Worker 미수정.
- 카드 내용 = 백서 §16 카드 UI 가이드. 카드 표현 = v2 스크린샷 (root index.html 참고). 운영 기준 = 빗썸 메인 + 한글 우선.
...
- .ws3-card-header (시안 dot + glow / 코인명 mono 700 / 큰 점수 cyan)
- .ws3-card-chips + .ws3-chip variant 17종 (grade-SP/S/A/B/C, mode-hot/watch/low, candidate, exchange-bit/upb/bin, structure, volume, risk, neutral, time)
- .ws3-signal-bar + .ws3-signal-fill (cyan→green gradient + glow)
- .ws3-card-meta (mono muted, flex wrap)
- .ws3-card-body-toggle + .ws3-card-body (.expanded로 표시)
- .ws3-card-section (.collapsed로 접힘) + .ws3-card-section-title (cyan) + .strong green / .weak red / .dim muted
- .toggle-mini 섹션별 펼침

신규 JS 함수:
- WS3_CHIP_KR_MAP — reasonChips 한글 매핑 7종
- ws3MapChipKr(code) — chip 코드 → {label, kind, orig}
- ws3RenderChipsKr(chips, max) — chip 배열 → HTML
- ws3ExchangeChipClass(exchange) / ws3ExchangeLabel(exchange)
- ws3OrLevelKr(level) — HOT_REVIEW/WATCH_REVIEW/LOW_SIGNAL → "HOT 검토"/"WATCH 관찰"/"LOW 관망"
- ws3GradeChipClass(grade) / ws3GradeLabel(grade)
- ws3SplitStrengthsWeaknesses(chips) — kind 기반 분리
- ws3BuildBriefing(r) — selectionReason.summary 1문장
- ws3BuildCardBody(r, req, idx) — 백서 §16.4 18 섹션 slot HTML

reasonChips 한글 매핑:
- VOLUME_SURGE → 거래량 증가 (volume)
- LOW_VOLUME → 거래량 부족 (risk)
- HIGH_CLOSE_POSITION → 종가 상단 (structure)
- UPPER_WICK_RISK → 윗꼬리 리스크 (risk)
- WIDE_RANGE_RISK → 변동폭 위험 (risk)
- SHORT_MOMENTUM → 단기 모멘텀 (volume)
- POSITIVE_CHANGE → 상승 전환 (volume)

빗썸 메인 정렬:
- 거래소 select 3 spots (Section 6/7/8): bithumb (메인) / upbit (보조, selected) / binance (참고)
- label 보조: "(빗썸 메인 · 업비트 보조 · 바이낸스 참고)"
- Dev Open Banner 상태 배지 4종 (준비 / 개발 개방 모드 / 수동 운영 / 자동 알림 꺼짐) + "메인 거래소: 빗썸" 강조 line
- 카드 chip 색 v2 톤 그대로: .exchange-bit #ff7849 / .exchange-upb #5da9ff / .exchange-bin #f0b90b
- 카드바디 §1 핵심 요약: "거래소: 빗썸" 표시

mcRenderResults 전면 교체:
- 카드 헤더: 좌측 네온 라인 + dot + 코인명 + 큰 점수 + chip 묶음 + 신호 강도 바 + 메타 row
- 카드바디 §16.4 18 섹션 모두 slot 표시 (구현 8개 + 후속 10개)
- 펼침 토글 ("근거 펼치기 ▾" ↔ "근거 접기 ▴") + 섹션 mini toggle

백서 §16.4 18 섹션 적용:
- 구현: §1 핵심 요약 / §2 카드헤더 해석 / §3 점수 요약 / §3.5 선별 근거·종합 브리핑 / 강점·약점 / §4 구조 판단 / §5 매수세·수급 / §12 지표 상세 / §18 고급 상세
- slot 표시 (후속): §6 시장상황·BTC / §7 전략 성향 / §8 진입 가이드 / §9 매도전략 A-F (6 slot) / §10 세력 판단 / §11 반복신호 / §13 코인 메타 / §14 뉴스 / §15 시장 참고 / §16 외부 보조 / §17 후보 검증
- 섹션 삭제 0건, 미구현은 "데이터 수집 중" / "계산 대기" / "후속 단계" 안내

Section 11 (선택 후보 미리보기):
- 한글 라벨: 선택 마켓 / 거래소·기준봉 / 점수·등급 / 운영자 검토 레벨 / 후보 자격(예/아니요) / 운영자 검토(예/아니요) / 근거(한글) / 최근 시각 / 종가 / 변화율·거래량 배수 / 위험 메모
- 거래소 한글 라벨 (ws3ExchangeLabel) 적용
- ws3MapChipKr 한글 chip 매핑 적용
- 위험 메모 한글: UPPER_WICK_RISK → 윗꼬리 리스크 / WIDE_RANGE_RISK → 변동폭 위험 / LOW_VOLUME → 거래량 부족

가져온 것 (UI 표현만):
- v2 색상 톤 (.grade-SP gold / .grade-S pink / .grade-A red / .grade-B orange)
- v2 exchange chip 색 (.exchange-bit/upb/bin)
- v2 시안 네온 + glow (v0.32.2 누적)
- v2 좌측 네온 라인 카드 shell
- v2 점수 큰 표시
- v2 신호 강도 바
- v2 펼침 카드 패턴
- v2 카드 섹션 박스
...
가져오지 않은 것 (의도적):
- v2 관심 / 정밀 / 표준 / 24h / 진행도 의미
- v2 active tracking / completed history
- v2 tracking pipeline / 분석기 state
- v2 ViewModel 의미값 / 데이터 구조
- v2 수익/손실 판정 의미
- SPLUS / splus 사용자 노출 (코드 주석에서도 제거)
- 전통 캔들명 (도지 / 망치 / 장악형) 사용자 UI 라벨 + 코드 주석

미구현 (v0.32.x / v0.33 후속):
- 상단 탭 (스캐너 / 추적 분석)
- 검색창 + 정렬 버튼 (최신순/점수순/후보순/마감임박)
- 큰 "스캔 시작" 중심 버튼 (v2 SCAN START 감성)
- 빗썸 adapter 신규 구현 (v0 Bithumb-only 기준)
- 카드바디 미구현 모듈 점진 구현

필수 정적 테스트:
- diff -q web↔index 0건 (byte-for-byte mirror)
- Embedded <script> parse: 2 blocks ALL_BLOCKS_OK
- 보호 파일 (workers / worker.js / wrangler.toml / index.html / manifest.json / service-worker.js / v3/ 25종 / CODE_CONTRACT / WORKFLOW_TEMPLATE / KV adapter / wrangler-canary.example.toml / .gitignore) diff 0건
- 영어 UI label 잔존 grep 0건 (Operator Dashboard Summary 등 8 표현)
- 금지 표기 grep 0건 (SPLUS / splus / 도지 / 망치 / 장악형) — 코드 주석에서도 제거
- v2 금지 의미 grep 0건 (정밀 / 표준 / 24h / 진행도)
...
본 commit 시점 Cloudflare 변경 0건 / 실 호출 0건

후속:
- push 후 Pages redeploy 1회 (Worker 미수정 → Worker redeploy 불필요)
- 자연검증: console 접속 → 32-market scan → v2 카드 shell / 백서 §16 슬롯 / 빗썸 메인 / 한글 chip / 카드 펼침 / 종합 브리핑 / 강점·약점 시각 확인
- Limited Live 실 발송은 발송 로직 미변경으로 필수 아님
- 다음 후보: 상단 탭 / 검색 / 정렬 / 스캔 시작 중심 버튼 / 빗썸 adapter / 카드바디 미구현 모듈 점진 구현 / Cron·auto Telegram·candidate 저장·tracking 시작은 별도 사용자 명시 승인 전까지 계속 disabled
```

## v0.32.2 핵심 메모

```text
- v0.32.2 = v2 UI 톤 이식 + 한글 우선 문구 정렬. 백서 정렬 / 기능 변경 0건 / Worker 미수정.
- 운영 상태 유지: WS3_LIMITED_LIVE_ENABLED=true / Cron disabled / 자동 알람 disabled / candidate 저장 disabled / tracking 시작 disabled
- 사용자 명시 방향: "v2 감성 계승 / 불필요한 영어 제거 / 실사용 완성도 상승"

가져온 것 (root index.html read-only 참고):
- 다크 네이비 배경 (--bg-primary #080d13)
- 글래스 카드 (--bg-card #0f1a27 + border)
- 시안 네온 (--accent-cyan #00d4ff)
- glow-cyan / glow-green / glow-red / glow-orange
- 32px×32px 시안 그리드 배경 (body::before)
- JetBrains Mono + Noto Sans KR
- 카드 shell / chip 톤 / 버튼 톤 / 상태 색상

가져오지 않은 것:
- v2 관심 / 정밀 / 표준 / 24h 퍼센트 / 진행도 의미
- v2 active tracking / completed history / 분석기 state pipeline
- v2 데이터 구조 / class 이름 / 로직
- WS3 독립 앱 유지

한글 우선 정리:
- h1: WS3 Canary Console → WS3 운영 콘솔
- Section 헤더 11개 모두 한글 (예: 1. 설정 / 8. 다종목 스캔 / 11. 운영자 검토 발송)
- Sub-panel: 운영 요약 / 상위 후보 5개 / 후보 목록 / 실패 마켓 / 필터 / 최근 스캔 기록 / 선택 후보 미리보기 / 텔레그램 미리보기 / 최근 발송 기록
- Dashboard card: HOT_REVIEW/WATCH_REVIEW/LOW_SIGNAL → HOT/WATCH/LOW (badge 축약)
- Input label: 거래소 / 마켓 / 타임프레임 / 캔들 수 / 확인 문구 등
- Button label: 상태 확인 / 카나리 발송 / 운영자 리셋 / 시세 미리보기 실행 / 단일 코인 점수 실행 / 스캔 실행 / 업비트 원화 프리셋 불러오기 / 테스트 알림 발송 / 제한 운영 알림 보내기
- JS: statusEl 'status:' → '상태:' / Section 11 selector placeholder 한글
- 영어 병기: 운영자 검토(Operator Review) / 제한 운영(Limited Live) / 개발 개방(Dev Open)

유지 (의도적):
- Worker raw key (mode/messageType/code/score/grade/reasonChips/telegramSent/kvWritten/kvWriteScope/candidateStored/trackingStarted 등) — 디버깅·매핑 일관성
- P-S / P-A / P-B / P-C 등급 코드 — 기술 용어, 백서 허용
- 구조값 chip (VOLUME_SURGE / HIGH_CLOSE_POSITION / LOW_VOLUME / UPPER_WICK_RISK 등) — 백서 §15
- ID / class / JS 변수 / 함수 / Worker 응답 매핑 모두 그대로

백서 기준 준수:
- 디자인 톤 + ViewModel 분리 철학만 이식
- v2 데이터 의미 미이식
- WS3 독립 앱 유지
- 사용자 표시 등급 P-S/P-A/P-B/P-C + S+ 표기 정책 (SPLUS / splus 사용자 노출 0건)
- 전통 캔들명 (도지 / 망치 / 장악형) 사용자 UI 라벨 0건
- 구조값 중심 표현 유지

필수 정적 테스트:
- diff -q web/ws3-canary-console.html web/ws3-canary-console/index.html → 0건
- Embedded <script> parse: 2 blocks ALL_BLOCKS_OK
- 보호 파일 diff 0건 (workers / worker.js / wrangler / index.html / manifest.json / service-worker.js / v3/ / CODE_CONTRACT / WORKFLOW_TEMPLATE / KV adapter / wrangler-canary.example.toml / .gitignore)
- 영어 UI label 잔존 grep 0건 (Operator Dashboard Summary / Top 5 Fixed Candidates / Failed Market List / Filter Tabs / Selected Candidate Preview / Telegram Message Preview / Memory-only Scan History / Memory-only Sent History)
- 금지 표기 grep 0건 (SPLUS / splus / 도지 / 망치 / 장악형)
- 노출된 폐기 hash repo-wide 매치 0건
- bot_token / chat_id / message_id / Invoke Token / SHA-256 hash / KV namespace ID / raw Telegram response / raw exchange full response — 정책 문맥만
...
후속:
- push 후 Pages redeploy 1회만 (Worker 미수정 → Worker redeploy 불필요)
- 자연검증: console 접속 → 32-market scan → v2 톤 / 한글 문구 / dashboard / Top 5 / filter / preview / sent history 시각 확인
- 다음 후보: Bithumb 전환 / 등급 정렬 / structureBucket / signalCycle / 자연검증 후 톤·문구 미세 조정 / 모바일 UX 자연검증 / Cron·auto Telegram·candidate 저장·tracking 시작은 별도 사용자 명시 승인 전까지 계속 disabled
```

## v0.32.1 Dev Open Operator UX — Live Validation Success

```text
- invite/token/endpoint input removed for dev-open 실사용 단계
- console immediate entry confirmed
- scan flow reachable
- Limited Live remains manual only
- final security hardening deferred
```

## v0.32.1 핵심 메모

```text
- v0.32.1 = No Invoke Token / Dev Open Operator UX Patch. 보안 강화 아님 — 실사용 UX 속도 개선.
- 사용자 명시 방향: "지금은 속도, 보안은 final deploy에서 다시 붙임. Invoke Token 입력 제거. v2처럼 접속 → 바로 사용."
- 운영 상태 유지: WS3_LIMITED_LIVE_ENABLED=true / Cron disabled / 자동 알람 disabled / candidate 저장 disabled / tracking 시작 disabled
- web/ws3-canary-console.html 변화 / web/ws3-canary-console/index.html byte-for-byte mirror 유지
- workers/ws3-telegram-canary-worker.js 변화 (helper + 5 endpoint wrap)

Worker 변경:
- 신규 상수: WS3_OPERATOR_AUTH_MODE = 'DEV_OPEN'
- 신규 helper: isDevOpenOperatorRoute(pathname) — 5 routes (/state, /multi-candidate-dry-run, /send-limited-live-alert, /live-preflight, /candidate-dry-run)
- 5 dev-open endpoint token check를 `if (!isDevOpenOperatorRoute(path)) { ... }` 로 wrap (기존 코드 보존)
- 4 잠금 endpoint token check 그대로:
  · /send-canary (Telegram 실 발송 위험)
  · /cleanup-confirm (cleanupRequired KV write)
  · /operator-reset (alreadySent flip + resetCount + KV write)
  · /send-candidate-test (Telegram 발송, normal/forced)
- 잠금 endpoint 호출은 web에서 token=null로 시도되므로 client-side 또는 worker가 MISSING_INVOKE_TOKEN 차단

Web Console 변경:
- <main id="consoleApp" hidden> → <main id="consoleApp"> (접속 즉시 main 표시)
- <section id="inviteGate" ... hidden> (코드 보존, final 게이트 재활성화 용이)
- Section 1 Configuration UI 전환:
  · Worker Endpoint input (type=text) → panel display + <input type="hidden" value="https://ws3-telegram-canary.neosiwon.workers.dev">
  · Invoke Token input (type=password) → <input type="hidden" value="">
  · "Operator Mode: DEV_OPEN / Token input: disabled" 상태 표시
- Dev Open Banner (Section 1 위) 추가:
  · "Operator Mode: DEV OPEN | Invoke Token input disabled | Endpoint preset"
  · "Final security ... deferred to final deploy gate"
- JS:
  · WS3_DEFAULT_WORKER_ENDPOINT = 'https://ws3-telegram-canary.neosiwon.workers.dev'
  · safeEndpoint() → preset endpoint 반환
  · readTokenAndClear() → null 반환 (no-op)
  · postJson/getJson — token이 string && length > 0일 때만 X-WS3-Canary-Token header set
  · 5 dev-open button handler에서 `if (!token) MISSING_INVOKE_TOKEN` 제거
  · 4 잠금 button handler은 그대로 유지

[A] textarea label fix 통합 (v0.32 자연검증 minor finding):
- before: <label for="mc_markets">Markets (콤마 또는 줄바꿈, 최대 10)</label>
- after:  <label for="mc_markets">Markets (콤마 또는 줄바꿈, 최대 50)</label>
- v0.31.0-fix-1에서 worker MULTI_CANDIDATE_MAX_MARKETS=50 + web validation + request 모두 cap=50으로 갱신 시 label만 갱신 누락이었음

Token이 HTML/JS에 박혀있지 않은지 확인:
- HTML 상수 X / JS 상수 X / data-* 속성 X / postMessage X / localStorage X / sessionStorage X / indexedDB X / cookie X
- WS3_DEFAULT_WORKER_ENDPOINT (Worker URL, secret 아님) 는 프론트에 상수로 있음 — OK
...
| /send-limited-live-alert | POST | ✅ | skip | Origin + manualTrigger + confirmPhrase + WS3_LIMITED_LIVE_ENABLED + per-key KV guard |
...
- storage 검사: 정책 문맥만, 실 호출 0건
- token UI 사용자 입력 검사: `grep "type=\"text\".*id=\"endpoint\"|type=\"password\".*id=\"token\""` → 0건
- 노출된 폐기 hash repo-wide 매치 0건
- bot_token / chat_id / message_id / Invoke Token / SHA-256 hash / KV namespace ID / raw Telegram response / raw exchange full response — 정책 문맥만
...
후속:
- push 후 Worker redeploy (5 dev-open wrap 반영) + Pages redeploy (UI 변경 반영)
- 자연검증: 접속 즉시 main 표시 → 32-market scan 1회 → dashboard / top5 / filter / preview 확인
- Limited Live 실제 발송 필수 아님 (발송 로직 미변경)
```
...
```text
- v0.31.1 = 운영 가능한 최소 웹 콘솔의 첫 자연검증 안정화 patch. 새 기능 아님.
- 운영 상태 그대로: WS3_LIMITED_LIVE_ENABLED=true 유지 / Cron disabled / 자동 알람 disabled / candidate 저장 disabled / tracking 시작 disabled
- web/ws3-canary-console.html 1724 → 1731 라인 (+7), web/ws3-canary-console/index.html byte-for-byte mirror 유지
- 변경 1: WS3_OPERATOR_REVIEW_PRESET_40 의심 8종 (KRW-MATIC / KRW-RENDER / KRW-JUP / KRW-ONDO / KRW-PEPE / KRW-BONK / KRW-TIA / KRW-PYTH) 제거 → 32 markets
- 변경 2: UI button label "Load 40-market Upbit preset" → "Load Upbit KRW preset"
- 변경 3: Section 11 Limited Live Send handler safety fallback — Worker error response (LIMITED_LIVE_DISABLED 503 외 모든 error 분기) safety 누락 시 client renderer 명시 false 표시 (telegramSent / kvWritten / candidateStored / trackingStarted 모두 false, kvWriteScope만 "-" 의미 없음 표시)
- 변경 4 (skip): operator review card 가독성 추가 개편은 minimal patch 원칙 준수 위해 의도적 skip
- Worker 미수정: workers/ws3-telegram-canary-worker.js diff 0건, VERSION WS3_v0.31.0_web_first_minimum_operator_mode 유지
- node --check workers/ws3-telegram-canary-worker.js PASS (Worker 미변경 확인용)
- diff -q web/ws3-canary-console.html web/ws3-canary-console/index.html → 0건
- preset list count 32 (30~40 유지 요구사항 만족), 중복 0건, 제거 8종 모두 제외 확인
- maxMarkets cap 50 (worker MULTI_CANDIDATE_MAX_MARKETS=50 + web validation markets.length>50 + web request maxMarkets:50) 모두 유지
- 보호 파일 (worker.js / wrangler.toml / index.html / manifest.json / service-worker.js / v3/ 25종 / WS3_CODE_CONTRACT.md / WS3_WORKFLOW_TEMPLATE.md / workers/ws3-telegram-canary-worker.js / workers/ws3-canary-state-kv-adapter.js / wrangler-canary.example.toml / .gitignore) diff 0건
- 본 commit 시점 Cloudflare 변경 0건 (Worker 재배포 / Pages 재배포 / KV namespace / allowed origins / secrets / WS3_TELEGRAM_CANARY_ENABLED / WS3_CANDIDATE_TEST_ENABLED / WS3_LIMITED_LIVE_ENABLED 모두 0건)
- 실 호출 0건 (실 거래소 API / Telegram API / KV write / candidate 저장 / tracking 시작 모두 0건)
- 노출된 폐기 hash repo-wide 매치 0건, bot token / chat id / invoke token / KV namespace id / Telegram message_id / raw Telegram response / raw exchange full response 노출 0건
- 후속: push 후 Pages redeploy 1회만 진행 / Worker redeploy 불필요 / 사용자 자연검증 재개 (40 → 32 preset scan failCount 확인 + disabled response fallback 표시 확인)
- 다음 후보 (v0.31.x): preset 추가 정리 / preset 동적 fetch (사용자 승인 필요) / operatorReview score·chip 임계값 조정 / HOT_REVIEW·WATCH_REVIEW 카드 추가 시각 강조 / duplicate guard window 60s 적정성 자연검증 / 모바일 카드 UX / console hosting domain rotate 시점 가이드 / Cron·auto Telegram·candidate 저장·tracking 시작은 별도 사용자 명시 승인 전까지 계속 disabled
```
