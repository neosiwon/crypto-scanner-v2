...
async function fetchBithumbCoinMeta(deps) {
  ...
  if (COIN_META_BITHUMB_CACHE.marketAll && now < COIN_META_BITHUMB_CACHE.marketAllExpiry) {
    ...
    try {
      var r1 = await deps.fetchImpl('https://api.bithumb.com/v1/market/all?isDetails=true', { method: 'GET' });
      if (r1.ok) {
  ...
  // 2. tickerAll (매 스캔)
  try {
    var r2 = await deps.fetchImpl('https://api.bithumb.com/public/ticker/ALL_KRW', { method: 'GET' });
    if (r2.ok) {
      var j = await r2.json();
...
// §constants ───────────────────────────────────────────────────────────
var VERSION = 'WS3_v0.51.0_candidates_endpoint';
...
async function handleFetch(request, env, ctx, deps) {
  ...
  // GET /candidates.json  (v0.51.0 — 공개 GET / KV 읽기만 / 봇용 / origin·token 없음)
  //   Cron 이 적재한 KV 값 패스스루 (스캔 안 함, 즉시). 비어있으면 PENDING.
  if (path === '/candidates.json' && method === 'GET') {
    var candKv = (isPlainObject(resolvedDeps) && hasOwn(resolvedDeps, 'kv')) ? resolvedDeps.kv : (env ? env[KV_BINDING_NAME] : null);
    if (!candKv || typeof candKv.get !== 'function') {
      return jsonResponse({ ok: false, status: 'ERROR', code: 'PERSISTENCE_UNAVAILABLE', httpStatus: 503 }, 503, false, null);
    }
    var candJson;
    try { candJson = await candKv.get(CANDIDATES_KV_KEY); } catch (e) {
      return jsonResponse({ ok: false, status: 'ERROR', code: 'CANDIDATES_KV_READ_ERROR', httpStatus: 502 }, 502, false, null);
    }
    if (typeof candJson !== 'string' || candJson.length === 0) {
      return jsonResponse({ ok: false, status: 'PENDING', code: 'CANDIDATES_NOT_READY', httpStatus: 503, schemaVersion: CANDIDATES_SCHEMA_VERSION }, 503, false, null);
    }
    if (typeof Response === 'undefined') {
      return { _mockResponse: true, status: 200, body: candJson, headers: { 'Content-Type': 'application/json; charset=utf-8' } };
    }
    return new Response(candJson, { status: 200, headers: {
...
// ════════════════════════════════════════════════════════════════════════
// v0.51.0 — /candidates.json (업비트 수축 후보군 / Cron → KV → 공개 GET)
//   명세서 §10~12 / 방식1 봇 독립 / 동결 모듈 무변경 / 점수 로직 무변경
//   빗썸 runMultiCandidatePipeline 과 완전 분리 (회귀 0) — 신규 업비트 경로
//   봇이 GET /candidates.json → 수축(박스≤5%) 종목 받아 체결강도 감시
// ════════════════════════════════════════════════════════════════════════
var CANDIDATES_KV_KEY = 'ws3:candidates:latest';
var CANDIDATES_SCHEMA_VERSION = 'candidates-v1';
// 메이저 16종 (고정 — CAP_THRESHOLDS 아님 / 알파 21건에 메이저 0건 / ONDO 등 잘림 방지)
var CANDIDATES_MAJORS = ['BTC','ETH','XRP','USDT','USDC','SOL','DOGE','ADA','TRX','BCH','LINK','DOT','AVAX','SHIB','TON','BNB'];
var CANDIDATES_SCAN_CHUNK = 4;        // Pass1 동시 fetch (m5 1개/종목)
var CANDIDATES_BUILD_CHUNK = 2;       // Pass2 동시 종목 (종목당 m15+h1 2 fetch → 4 동시) — 업비트 10req/s 한도 회피
var CANDIDATES_SCAN_DELAY_MS = 300;   // 청크 간 지연 (~7 req/s 목표 / 429 degraded 점수 방지)
var CANDIDATES_M5_COUNT = 12;         // 직전 60분 = m5 12개 (수축 박스 측정)
var CANDIDATES_TF_COUNT = 60;         // 수축 통과분 정밀 스코어용 m15/h1 캔들 수

function candidatesSleep(deps, ms) {
  return new Promise(function(res) {
    if (deps && typeof deps.setTimeoutImpl === 'function') deps.setTimeoutImpl(res, ms);
    else res();
  });
}

// 업비트 유니버스: market/all(KRW, 메이저 제외) + ticker/all(거래대금 노이즈필터)
//   노이즈필터 = "죽은 종목 제외" 용도 (좁히는 용도 아님 / VOLUME_AS_CANDIDATE_FILTER false)
async function fetchUpbitUniverse(deps, cfg) {
  var alpha = (cfg && cfg.ALPHA_TRIGGER) || {};
  var minValue = (typeof alpha.MIN_VALUE_KRW === 'number') ? alpha.MIN_VALUE_KRW : 10000000;
  var majors = {};
  for (var mi = 0; mi < CANDIDATES_MAJORS.length; mi++) majors[CANDIDATES_MAJORS[mi]] = true;

  var mResp = await deps.fetchImpl('https://api.upbit.com/v1/market/all?isDetails=true', { method: 'GET' });
  if (!mResp || mResp.ok !== true) return { ok: false, code: 'UPBIT_MARKET_ALL_HTTP' };
  var mAll = await mResp.json();
  if (!Array.isArray(mAll)) return { ok: false, code: 'UPBIT_MARKET_ALL_PARSE' };
  var nameMap = {};
  for (var j = 0; j < mAll.length; j++) {
    var m = mAll[j];
    if (!m || typeof m.market !== 'string' || m.market.indexOf('KRW-') !== 0) continue;
    var sym = m.market.slice(4);
    if (majors[sym]) continue;
    nameMap[m.market] = {
      koreanName: (typeof m.korean_name === 'string') ? m.korean_name : null,
      warning: !!(m.market_event && m.market_event.warning)
    };
  }

  var tResp = await deps.fetchImpl('https://api.upbit.com/v1/ticker/all?quote_currencies=KRW', { method: 'GET' });
  if (!tResp || tResp.ok !== true) return { ok: false, code: 'UPBIT_TICKER_ALL_HTTP' };
  var tAll = await tResp.json();
  if (!Array.isArray(tAll)) return { ok: false, code: 'UPBIT_TICKER_ALL_PARSE' };
  var universe = [];
  for (var k = 0; k < tAll.length; k++) {
    var t = tAll[k];
    if (!t || typeof t.market !== 'string') continue;
    var info = nameMap[t.market];
    if (!info) continue; // 메이저/비KRW 제외됨
    var acc24 = Number(t.acc_trade_price_24h);
    if (!isFinite(acc24) || acc24 < minValue) continue; // 노이즈필터 (죽은 종목)
    universe.push({
      market: t.market,
      symbol: t.market.slice(4),
      koreanName: info.koreanName,
      warning: info.warning,
      accTradePrice24h: acc24,
      tradePrice: Number(t.trade_price),
      changeRate: Number(t.signed_change_rate)
    });
  }
  return { ok: true, universe: universe };
}

// 박스폭 = (최고-최저)/최저 × 100 (직전 60분 m5 12개)
function computeBoxPct(candles) {
...
// 수축 통과 종목 → m15/h1 fetch + v3 빌더 체인 재사용 → candidate 레코드
async function buildCandidateRecord(deps, item, cfg) {
  var tf = await Promise.all([
    fetchLiveCandles(deps, 'https://api.upbit.com/v1/candles/minutes/15?market=' + item.market + '&count=' + CANDIDATES_TF_COUNT, LIVE_PREFLIGHT_FETCH_TIMEOUT_MS),
    fetchLiveCandles(deps, 'https://api.upbit.com/v1/candles/minutes/60?market=' + item.market + '&count=' + CANDIDATES_TF_COUNT, LIVE_PREFLIGHT_FETCH_TIMEOUT_MS)
  ]);
  var m15 = [], h1 = [];
  if (tf[0].ok === true) { var n15 = normalizeCandles('upbit', tf[0].raw, CANDIDATES_TF_COUNT); if (n15.ok === true) m15 = n15.candles; }
  if (tf[1].ok === true) { var nh1 = normalizeCandles('upbit', tf[1].raw, CANDIDATES_TF_COUNT); if (nh1.ok === true) h1 = nh1.candles; }

  var candlesObj = { m5: item.m5, m15: m15, h1: h1, h4: [], d1: [] };
  var nowTs = (deps && typeof deps.nowMs === 'number') ? deps.nowMs : Date.now();
  var marketCtx = { exchange: 'UPBIT', market: item.market, ts: nowTs };

  var fp = globalThis.WS3_FeaturePayload_Builder.build(candlesObj, marketCtx);
  if (!globalThis.WS3_FeaturePayload.isValid(fp)) return null;
  var sb = globalThis.WS3_ScoreBreakdown.build(fp);
  var sd = globalThis.WS3_StructureBucket.build(fp, sb);
  var sc = globalThis.WS3_SignalCycle.build(fp, sb, sd, null);
  var sp = globalThis.WS3_StrategyPlan.build(fp, sb, sd, sc);
  var smz = calcSmartMoneyZone(sd, fp, cfg);
  var cp = calcCurrentPhase(smz, fp, cfg);
  var totalScore = (sb && sb.valid) ? sb.totalScore : 0;
  var meta = getCoinMeta(item.market, { marketAllMap: {}, tickerAll: {} }, cfg);

  return {
    symbol: item.symbol,
    market: item.market,
    koreanName: item.koreanName || (meta ? meta.koreanName : null),
    asOfTs: fp.ts,
    boxPct: item.boxPct,
    boxLookbackMin: (cfg && cfg.ALPHA_TRIGGER && cfg.ALPHA_TRIGGER.SQUEEZE_LOOKBACK_MIN) || 60,
    totalScore: totalScore,
    grade: classifyV3Grade(totalScore),
    strategyBias: sp ? sp.strategyBias : null,
    structureBucket: sd ? sd.structureBucket : null,
    priceZone: (sd && sd.priceZone) ? sd.priceZone.zone : null,
    cycleState: sc ? sc.cycleState : null,
    cyclePhase: sc ? sc.cyclePhase : null,
    currentPhase: cp ? cp.state : null,
    referencePrice: (typeof item.tradePrice === 'number' && isFinite(item.tradePrice)) ? item.tradePrice : null
  };
}

// 전체 스캔 (Cron 에서 호출) — Pass1 m5 수축필터 → Pass2 수축분 v3 스코어
async function runCandidatesScan(deps) {
  var cfg = globalThis.WS3_CONFIG || {};
  var alpha = (cfg && cfg.ALPHA_TRIGGER) || {};
  var boxMax = (typeof alpha.SQUEEZE_BOX_PCT_MAX === 'number') ? alpha.SQUEEZE_BOX_PCT_MAX : 5.0;
  var nowTs = (deps && typeof deps.nowMs === 'number') ? deps.nowMs : Date.now();

  var uni = await fetchUpbitUniverse(deps, cfg);
  if (uni.ok !== true) return { ok: false, code: uni.code, candidates: [], scanned: 0, squeezed: 0, generatedAt: nowTs };
  var list = uni.universe;

  // Pass 1: m5 박스폭 수축필터 (throttled)
  var squeezed = [];
  for (var s = 0; s < list.length; s += CANDIDATES_SCAN_CHUNK) {
    var chunk = list.slice(s, s + CANDIDATES_SCAN_CHUNK);
    var res1 = await Promise.all(chunk.map(function(item) {
      var url = 'https://api.upbit.com/v1/candles/minutes/5?market=' + item.market + '&count=' + CANDIDATES_M5_COUNT;
      return fetchLiveCandles(deps, url, LIVE_PREFLIGHT_FETCH_TIMEOUT_MS).then(function(fr) {
        if (fr.ok !== true) return null;
        var n = normalizeCandles('upbit', fr.raw, CANDIDATES_M5_COUNT);
        if (n.ok !== true) return null;
        var boxPct = computeBoxPct(n.candles);
        if (boxPct === null || boxPct > boxMax) return null; // 수축 아님 → 제외
        item.m5 = n.candles;
        item.boxPct = Math.round(boxPct * 100) / 100;
        return item;
      }).catch(function() { return null; });
    }));
    for (var ri = 0; ri < res1.length; ri++) if (res1[ri]) squeezed.push(res1[ri]);
    if (s + CANDIDATES_SCAN_CHUNK < list.length) await candidatesSleep(deps, CANDIDATES_SCAN_DELAY_MS);
  }

  // Pass 2: 수축 통과분만 m15/h1 + v3 체인 (throttled — 종목당 2 fetch라 청크 작게)
  var candidates = [];
  for (var q = 0; q < squeezed.length; q += CANDIDATES_BUILD_CHUNK) {
    var qchunk = squeezed.slice(q, q + CANDIDATES_BUILD_CHUNK);
    var res2 = await Promise.all(qchunk.map(function(item) {
      return buildCandidateRecord(deps, item, cfg).catch(function() { return null; });
    }));
    for (var bi = 0; bi < res2.length; bi++) if (res2[bi]) candidates.push(res2[bi]);
    if (q + CANDIDATES_BUILD_CHUNK < squeezed.length) await candidatesSleep(deps, CANDIDATES_SCAN_DELAY_MS);
  }

  // 정렬: totalScore 내림차순 (봇이 boxPct sweet 1.5~4% + cycleState 로 재우선순위)
  candidates.sort(function(a, b) { return (Number(b.totalScore) || 0) - (Number(a.totalScore) || 0); });
  return { ok: true, candidates: candidates, scanned: list.length, squeezed: squeezed.length, generatedAt: nowTs };
}

// candidates.json 스키마 직렬화 (config 단일진실원 / secret 제외 — 임계값만)
function buildCandidatesJson(scan) {
  var cfg = globalThis.WS3_CONFIG || {};
  return {
    ...
    exchange: 'upbit',
    config: {
      ALPHA_TRIGGER: cfg.ALPHA_TRIGGER || null,
      EXIT_PARAMS: cfg.EXIT_PARAMS || null,
      MARKET_REGIME: cfg.MARKET_REGIME || null
    },
    scanned: scan.scanned,
    squeezed: scan.squeezed,
    count: scan.candidates.length,
    candidates: scan.candidates
  };
}

// Cron scheduled 핸들러 — 스캔 → KV 적재 (GET 은 KV 읽기만)
async function handleScheduled(event, env, ctx, deps) {
  var resolvedDeps = isPlainObject(deps) ? deps : {};
  var nowMs = (typeof resolvedDeps.nowMs === 'number' && isFinite(resolvedDeps.nowMs))
    ? resolvedDeps.nowMs
    : ((typeof Date !== 'undefined') ? Date.now() : 0);
  var state = isPlainObject(resolvedDeps.state) ? resolvedDeps.state : CANARY_PROCESS_STATE;
  var kv = (isPlainObject(resolvedDeps) && hasOwn(resolvedDeps, 'kv'))
    ? resolvedDeps.kv
    : (env ? env[KV_BINDING_NAME] : null);
  if (!kv || typeof kv.put !== 'function') return { ok: false, code: 'PERSISTENCE_UNAVAILABLE' };

  var depsRes = buildWorkerRuntimeDeps(resolvedDeps, nowMs, state);
  if (depsRes.ok !== true) return { ok: false, code: depsRes.code };

  var scan;
  try {
    scan = await runCandidatesScan(depsRes.deps);
  } catch (e) {
    return { ok: false, code: 'CANDIDATES_SCAN_ERROR', error: String((e && e.message) || e) };
  }
  if (scan.ok !== true) return { ok: false, code: scan.code };

  try {
    await kv.put(CANDIDATES_KV_KEY, JSON.stringify(buildCandidatesJson(scan)));
  } catch (e) {
    return { ok: false, code: 'CANDIDATES_KV_WRITE_ERROR' };
  }
  return { ok: true, count: scan.candidates.length, scanned: scan.scanned, squeezed: scan.squeezed };
}
...
module.exports = {
  ...
  runCandidatesScan: runCandidatesScan,
  fetchUpbitUniverse: fetchUpbitUniverse,
  buildCandidateRecord: buildCandidateRecord,
  buildCandidatesJson: buildCandidatesJson,
  computeBoxPct: computeBoxPct,
  CANDIDATES_KV_KEY: CANDIDATES_KV_KEY,
  CANDIDATES_MAJORS: CANDIDATES_MAJORS,
